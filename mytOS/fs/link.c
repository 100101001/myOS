...
PUBLIC int do_unlink()
{
	...
	if (pin->i_cnt > 2) {	/* the file was opened */
...
int deletefile(struct inode * pin ,int inode_nr,struct inode * dir_inode)
{
	

	/*if (pin->i_mode != I_REGULAR) { 
		printl("{FS} cannot remove file , because "
		       "it is not a regular file.\n");
		return -1;
	}*/

	if (pin->i_cnt > 2) {	/* the file was opened */
		printl("{FS} cannot remove file , because pin->i_cnt is %d.\n", pin->i_cnt);
		return -1;
	}
...
int deletedirectoy(struct inode * pin ,int inode_nr,struct inode * dir_inode)
{
...
PUBLIC int do_unlink_my()
{
	...
	if(filetype==0)
	{
		if(deletefile(pin,inode_nr,dir_inode)<0)
		{
			return -1;
		}
	...
	else
	{
		...
		if (pin->i_mode != I_DIRECTORY) { /* can only remove rdirectory files */
			printl("{FS} cannot remove file %s, because "
		       "it is not a directory file.\n",
		       pathname);
			return -1;
		}

		/*if (pin->i_cnt > 1) {	
			printl("{FS} cannot remove directory %s, because pin->i_cnt is %d.\n",
		       pathname, pin->i_cnt);
			return -1;
		}*/
		//recursivedelete(pin,inode_nr);
		deletedirectoy(pin ,inode_nr,dir_inode);
	}	
}

void recursivedelete(struct inode * root,int inode_nr)
{

	printl("i am here");
	int dir_blk0_nr = root->i_start_sect;
	int nr_dir_blks = (root->i_size + SECTOR_SIZE - 1) / SECTOR_SIZE;
	int nr_dir_entries =
	  root->i_size / DIR_ENTRY_SIZE; /**
	...
	for (int i = 0; i < nr_dir_blks; i++) {
		RD_SECT(root->i_dev, dir_blk0_nr + i);
		pde = (struct dir_entry *)fsbuf;
		for (int j = 0; j < SECTOR_SIZE / DIR_ENTRY_SIZE; j++,pde++) {
			struct inode * child=get_inode(root->i_dev,pde->inode_nr);
			if (child->i_mode == I_REGULAR)
			{
				deletefile(child,pde->inode_nr,root);
			}
			else
			{
				recursivedelete(child,pde->inode_nr);
				deletefile(child,pde->inode_nr,root);
			}
			if (++m > nr_dir_entries)
				break;
