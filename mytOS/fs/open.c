...
PUBLIC int do_open()
{
	...
	if(pathname[0]==0)
	{
		...
		printallchildfiles(root_inode,0);
		printallchildfiles(root_inode,0);
		printallchildfiles(root_inode,0);		    				printallchildfiles(root_inode,0);
		printallchildfiles(root_inode,0);
		printallchildfiles(root_inode,0);
		printallchildfiles(root_inode,0);
		printallchildfiles(root_inode,0);
	...
	if (pin) {
		...
		if (imode == I_CHAR_SPECIAL) {
			...
			printallchildfiles(pin,0);
			printallchildfiles(pin,0);
			printallchildfiles(pin,0);		    						printallchildfiles(pin,0);
			printallchildfiles(pin,1);
			assert(pin->i_mode == I_DIRECTORY);
			pin->i_cnt=1;
		}
...
PRIVATE struct inode * create_file(char * path, int flags)
{
	...
	//printl("%s'i_node number is %d",path,inode_nr);
...
void printallchildfiles(struct inode * dir_inode,int ifprint)
{
	...
	for (int i = 0; i < nr_dir_blks ; i++) {
		...
		for (int j = 0; j < SECTOR_SIZE / DIR_ENTRY_SIZE; j++,pde++) {
			...
			{
				/*if(memcmp("abc", pde->name, MAX_FILENAME_LEN) == 0)
				{
					printl("byte is right");
				}*/
				/**char * s;
				memcmp(s, pde->name, MAX_FILENAME_LEN);*/
				if(ifprint)
					printl("file:%s \n",pde->name);
			...
			{
				put_inode(temp);
				//printl("after put inode %s, cnt is %d",pde->name,temp->i_cnt);
			}
			//printl("%s cnt is %d",pde->name,temp->i_cnt);
			if (++m > nr_dir_entries)
				break;
		}
